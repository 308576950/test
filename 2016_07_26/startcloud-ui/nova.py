...
def server_list_paged(request, search_opts=None, all_tenants=False,
                      paginate=False, marker=None, sort_dir='desc',
                      sort_key='id', reversed_order=False):
    c_nova = novaclient(request)
    page_size = utils.get_page_size(request)
    servers = []
    has_prev_data = False
    has_more_data = False
    ...
    if search_opts is None:
        search_opts = {}
    if 'paginate' in search_opts.keys():
    ...
    _servers = c_nova.servers.list(True, search_opts=search_opts)
    if paginate:
        _servers, has_more_data, has_prev_data = update_pagination(
            _servers, page_size, marker, reversed_order)

    for server in _servers:
        servers.append(Server(server, request))

    return servers, has_more_data, has_prev_data


def server_list(request, search_opts=None, all_tenants=False):
    servers, has_more_data, _ = server_list_paged(request, search_opts,
                                                  all_tenants)
